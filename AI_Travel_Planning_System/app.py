...
with st.sidebar:
    ...
    for step in ["① Travel-Parser Agent", "② Flight Agent", "③ Hotel Agent", "④ Research Agent", "⑤ Final Agent"]:
...
if generate:
    if not user_query.strip():
        ...
        collected = {"flight_results": "", "hotel_results": "",
                     "research_results": "", "final_response": "", "llm_calls": 0}
        ...
        for chunk in app.stream(
            ...
            for node_name, state_update in chunk.items():
                ...
                with st.status(f"{icon}  {label}", state="complete", expanded=False):
                    if node_name == "travel_parser_agent":
                        text = state_update.get("user_query", "")
                        collected["user_query"] = text
                        st.markdown(text or "_No travel details extracted._")
